...
class Grid(object):
    def __init__(self, width=0, height=0, matrix=None, inverse=False, wrap=False):
        """
    ...
    def neighbors(self, node, diagonal_movement=DiagonalMovement.never):
        ...
        if self.wrap:
            y_less = (y - 1) % self.height
            y_more = (y + 1) % self.height
            x_less = (x - 1) % self.width
            x_more = (x + 1) % self.width
        else:
            y_less = (y - 1)
            y_more = (y + 1)
            x_less = (x - 1)
            x_more = (x + 1)
        # ↑
        if self.walkable(x, y_less):
            neighbors.append(self.nodes[y_less][x])
            s0 = True
        # →
        if self.walkable(x_more, y):
            neighbors.append(self.nodes[y][x_more])
            s1 = True
        # ↓
        if self.walkable(x, y_more):
            neighbors.append(self.nodes[y_more][x])
            s2 = True
        # ←
        if self.walkable(x_less, y):
            neighbors.append(self.nodes[y][x_less])
            s3 = True
        ...
        # ↖
        if d0 and self.walkable(x_less, y_less):
            neighbors.append(self.nodes[y_less][x_less])

        # ↗
        if d1 and self.walkable(x_more, y_less):
            neighbors.append(self.nodes[y_less][x_more])

        # ↘
        if d2 and self.walkable(x_more, y_more):
            neighbors.append(self.nodes[y_more][x_more])

        # ↙
        if d3 and self.walkable(x_less, y_more):
            neighbors.append(self.nodes[y_more][x_less])
