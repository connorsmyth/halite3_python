...
from pathfinding.core.heuristic import manhatten, octile, world_wrap
...
class Finder(object):
    ...
    def apply_heuristic(self, node_a, node_b, heuristic=None, width = 0, height = 0):
        """
    ...
    def process_node(self, node, parent, end, open_list, open_value=True, width = 0, height = 0):
        '''
        ...
        if not node.opened or ng < node.g:
            node.g = ng
            node.h = node.h or \
                     self.apply_heuristic(node, end, width=width, height=height) * self.weight
            # node.h = node.h or \
            #          self.apply_heuristic(node, end, width=width, height=height)
